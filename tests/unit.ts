...
import { clampZoom, wheelZoom } from "../src/composables/zoom";
...
assert.equal(wheelZoom(100, -80), 110);
assert.equal(wheelZoom(100, 80), 90);
...
for(const ext of extensions)assert.ok(associations.includes(`Software\\Classes\\.${ext}\\OpenWithProgids`));
assert.ok(!associations.includes('UserChoice'));
assert.ok(!/WriteRegStr[^\n]+"Software\\Classes\\\.[^"\\]+"/.test(associations));
assert.ok(associations.includes('"%1"'));
console.log('PASS installer: Open With supports all formats and preserves default associations');
