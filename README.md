...
- PDF：连续阅读、准确的长文档页码定位、可见的搜索高亮、文字划选与复制、目录和缩略图、密码解锁、临时旋转；旋转或调整页面适配后文字与链接仍对齐。放大后可按住空格并用左键拖动，或直接用鼠标中键拖动页面。缩略图有小/中/大三档，也能拖动导航栏边界调宽；阅读时目录高亮会跟随当前章节。选中文字后可高亮或写备注，批注仅保留在本次预览内，关闭文件即清空。高倍率预览会自动限制画布像素占用，可能比原始像素稍柔和；打印仍使用独立的打印渲染流程。
