...
预览工具栏会在窄窗口自动换行；PDF 和文本搜索在独立搜索行展开，支持 Esc 关闭，不挤走打印与全屏入口。Excel 工作表与定位/冻结操作分区显示，表格内容铺满可用区域。Markdown 表格、引用和代码块以及文本高亮适配白天/黑夜主题。

批量选择文件时先读取文件名和大小，切换到某个标签才加载其内容；离开标签会释放已渲染页面，下次切回时重新从本机读取。打印列表默认折叠新增条目，展开时才准备纸张预览。大文件或大量窗口仍会受本机可用内存影响。
...
- 多标签：固定宽度标签、悬停全名、拖动排序、跨窗口拖拽合并；标签栏滚轮或滚到文顶/底可切换文件。多开时加载排队；**已打开标签全部缓存**（约 512MB 内存预算内），切换秒开；非激活标签闲置后只卸预览 DOM，不丢已缓存字节
- 顶部「+ 打开」/ `Ctrl+O` 在当前窗口继续添加文件
- 连续滚动阅读 + 页码跟随；适合宽度 / 整页适配 / 自定义缩放 25%–400%
- 沉浸全屏（F11 / Esc），边角唤出工具条与页码
- PDF：连续阅读、准确的长文档页码定位、可见的搜索高亮、文字划选与复制、目录和缩略图、密码解锁、临时旋转；旋转或调整页面适配后文字与链接仍对齐。放大后可按住空格并用左键拖动，或直接用鼠标中键拖动页面。缩略图有小/中/大三档，也能拖动导航栏边界调宽；阅读时目录高亮会跟随当前章节。选中文字后可高亮或写备注，批注仅保留在本次预览内，关闭文件即清空。长文档按可见页绘制，滑出视野后释放画布，返回时重新绘制；高倍率预览限制画布像素占用，极高倍率下可能略显柔和。打印使用独立的打印渲染流程。
...
- 选择非 A4 或虚拟打印机时，会**提前提示**可能不支持该纸张（提示不阻塞打印）  
