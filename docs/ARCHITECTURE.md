...
PDF 的 `index.ts` 通过可选 `toolbar` 注册专属打印入口。`src/modules/pdf/print/` 包含打印面板、内存任务列表、独立打印视图与错误文案；其他模块不注册打印入口。`electron/printing/` 负责打印机枚举、PDF 文件选择、全局顺序提交和隐藏窗口生命周期。`shared/printing.ts` 定义纸张、页码与参数校验。
