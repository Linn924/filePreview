...
PDF 页面只为可见窗口附近排队绘制：同一页最多保留一个待执行任务，完成后若画布已替换或页面仍空白则限次补绘。任务与具体画布绑定，旧画布的离屏回调不能取消新画布；离屏画布释放像素内存。页面尺寸后台扫描只在更新可见页尺寸时重建文字层，避免远处页面尺寸变化反复触发文本解析。搜索每页批量发布命中，页面文字缓存按最近使用保留 32 页。预览画布沿用单页 600 万像素上限，打印走独立渲染路径。

## PDF 本次交互

`src/modules/pdf/usePan.ts` 只管理 PDF 滚动容器的空格/中键指针平移，并在输入框聚焦时保留正常空格输入。`PdfNav.vue` 内部持有本次侧栏宽度、缩略图尺寸和当前章节定位；缩略图尺寸改变时失效旧画布再重绘。`useAnnotations.ts` 与 `PdfNotes.vue` 保存选中文字后的临时矩形、备注内容与旋转角度；按页归一化矩形用于调整适配/旋转后的显示。状态随文件的内存 `view.pdfNotes` 在标签切换时保留，不写回 PDF 或磁盘。双页阅读未包含在本批次。

## PDF 页槽与文字坐标

PDF 虚拟滚动的累计高度包含每页真实高度和模块 CSS 的 24 px 页间距，滚动容器顶部 26 px 内边距在定位时单独扣除。文字层和链接注解使用与画布相同的页面旋转角度与当前显示宽度计算坐标；缩放、适配、旋转和页面尺寸变化会递增几何修订号，舍弃过期的异步构建结果。文字层保持透明字形，搜索命中仅绘制可见背景，避免覆盖原 PDF 字形。
