...
独立打印窗口与每文件参数：printing 套件检查窗口非模态且可缩放、原窗口没有遮罩、原页/纸张尺寸显示；两文件以 A5/指定页和 A4/两份分别提交，未勾选文件不打印，应用到全部复制而不共享可变设置。检查点击文件名切换预览、预览关闭后重建、重复打开同一 Tab 不重复添加、“并排查看”窗口不重叠。仅模拟实体打印出口，不实际出纸。
