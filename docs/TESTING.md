...
PDF 打印：`npm test -- --suite printing`。测试拦截打印机枚举与实体打印入口，不会向打印机发送任务；真实隐藏窗口完整渲染打印页，再用 Chromium 在内存生成 PDF，检查 A5 的 148 × 210 mm 纸张、页数、指定源页、批量任务及无效页码不提交。其他格式不得出现打印按钮。基础测试还检查 A5 横纵向尺寸、范围解析和选项校验。
